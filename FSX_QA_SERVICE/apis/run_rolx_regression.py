...
def insert_regression_record(task_id, creator, status, create_time):
    ...
    try:
        ...
        insert_sql = \
            ...
            "VALUES (%s, %s, %s, %s, %s)"
        insert_values = (task_id, status, creator, create_time, 2)
...
# 下载 rolx_report.log
@app_run_rolx_regression.route('/api/rolx_regression_list/download_rolx_log', methods=['POST'])
@jwt_required()
...
# 下载 rolx_report.xlsx
@app_run_rolx_regression.route('/api/rolx_regression_list/download_rolx_report', methods=['POST'])
@jwt_required()
